    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
        <div>
          <p style={{ margin: 0, fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", opacity: 0.85 }}>
            {tx("Today", "Hari ini")} · {status?.dayKey ?? "—"} {demoMode ? tx("(demo)", "(demo)") : ""}
          </p>
          <h2 style={{ margin: "6px 0 0", fontSize: compact ? 20 : 26, fontWeight: 900, display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {punchedIn ? tx("Punched in", "Sudah masuk") : tx("Punched out", "Sudah pulang")}
          </h2>
          <p style={{ margin: "8px 0 0", fontSize: 13, opacity: 0.9, display: "flex", alignItems: "center", gap: 6 }}>
            <MapPin style={{ width: 14, height: 14 }} />
            {tx("Multiple IN/OUT allowed — closed segments sum to working hours", "Boleh IN/OUT berulang — segmen tertutup dijumlahkan sebagai jam kerja")}
          </p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 12, marginTop: 20 }}>
        <div style={{ background: "rgba(255,255,255,0.12)", borderRadius: 14, padding: 14 }}>
          <p style={{ margin: 0, fontSize: 10, fontWeight: 700, opacity: 0.8, textTransform: "uppercase" }}>{tx("Live timer", "Timer live")}</p>
          <p style={{ margin: "6px 0 0", fontSize: 22, fontWeight: 900, fontFamily: "var(--font-mono)" }}>
            {punchedIn ? status?.liveLabel ?? "0s" : "—"}
          </p>
        </div>
        <div style={{ background: "rgba(255,255,255,0.12)", borderRadius: 14, padding: 14 }}>
          <p style={{ margin: 0, fontSize: 10, fontWeight: 700, opacity: 0.8, textTransform: "uppercase" }}>{tx("Closed today", "Tertutup hari ini")}</p>
          <p style={{ margin: "6px 0 0", fontSize: 22, fontWeight: 900, fontFamily: "var(--font-mono)" }}>
            {status?.totalClosedLabel ?? "0s"}
          </p>
        </div>
        <div style={{ background: "rgba(255,255,255,0.12)", borderRadius: 14, padding: 14 }}>
          <p style={{ margin: 0, fontSize: 10, fontWeight: 700, opacity: 0.8, textTransform: "uppercase" }}>{tx("Total (incl. live)", "Total (termasuk live)")}</p>
          <p style={{ margin: "6px 0 0", fontSize: 22, fontWeight: 900, fontFamily: "var(--font-mono)" }}>
      ...
      {status && status.segments.length > 0 && (
        <div style={{ marginTop: 18 }}>
          <p style={{ margin: "0 0 8px", fontSize: 11, fontWeight: 800, textTransform: "uppercase", opacity: 0.8 }}>
            {tx("Segments today", "Segmen hari ini")}
          </p>
          <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 6 }}>
            {status.segments.map((seg, i) => {
              ...
              return (
                ...
                >
                  <span>
                    {inAt?.toLocaleTimeString() ?? "—"} → {outAt ? outAt.toLocaleTimeString() : tx("running", "berjalan")}
                  </span>
