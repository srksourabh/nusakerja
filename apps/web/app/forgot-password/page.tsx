      ...
      <header className="max-w-6xl w-full mx-auto flex items-center justify-between z-10 py-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/login" className="text-xs font-bold text-slate-300 hover:text-white flex items-center gap-1.5 transition-colors">
            ...
            <span>{tx("Back to login", "Kembali ke Login")}</span>
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-md w-full mx-auto my-auto z-10 py-12">
        <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-6">
          <div className="text-center space-y-2">
            ...
            <h1 className="text-2xl font-black text-white">{tx("Forgot password?", "Lupa Kata Sandi?")}</h1>
            <p className="text-xs text-slate-400">{tx("Enter your registered company email to receive a password reset link.", "Masukkan email perusahaan terdaftar Anda untuk menerima tautan validasi dan reset kata sandi.")}</p>
          </div>
          ...
          {sent ? (
            <div className="p-6 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl text-center space-y-4 animate-in fade-in">
              ...
              <p className="text-xs text-emerald-300 font-semibold leading-relaxed">
                {tx("Password reset instructions were sent to {email}.", "Instruksi reset kata sandi telah dikirim ke {email}.", { email })}
              </p>
              <Link href="/login" className="inline-flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs transition-colors">
                <span>{tx("Back to login", "Kembali ke Halaman Login")}</span>
            ...
            <form onSubmit={handleResetSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">
                  {tx("Registered company email", "Email Perusahaan Terdaftar")}
                </label>
              ...
              >
                {loading ? (
                  <span>{tx("Sending reset link...", "Mengirimkan Tautan Reset...")}</span>
                ) : (
                  <>
                    <span>{tx("Send email validation link", "Kirim Tautan Validasi Email")}</span>
                    <ArrowRight className="w-4 h-4" />
                  </>
                )}
              </button>
